...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: highPriorityPatients, error: patientsError } = await supabase
      ...
      `)
      .gte('calculate_patient_priority(chronic_conditions, allergies, current_medications)', 3)
      .order('calculate_patient_priority(chronic_conditions, allergies, current_medications)', { ascending: false })
    ...
    const transformedPatients = highPriorityPatients.map(patient => ({
      ...
      medication_count: (patient.current_medications?.match(/\n/g) || []).length + 1
    }))
